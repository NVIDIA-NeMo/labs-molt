...
class RolloutRayActor:
    ...
    async def reset_prefix_cache(self):
        ...
        Keep-mode pause is not a reliable substitute: vLLM versions differ on whether
        it clears anything (0.29 preempts in-flight requests and clears caches by
        default, while its /pause contract documents clear_cache as ignored in keep
        mode), so a request straddling the update can still hold KV from the previous
        policy. Ask for the running requests and any external KV connector too, and say
        so when the engine still refuses (it returns False rather than raising).
        """
        import inspect
