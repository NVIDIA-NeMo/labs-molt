...
MODEL_PATH="${MODEL_PATH:?Set MODEL_PATH to a Qwen3.5-35B-A3B checkpoint.}"
...
SAVE_ROOT="${SAVE_ROOT:-$REPO_ROOT/outputs/quick_start-sft-qwen3-5-lora/run}"
...
torchrun --standalone --nproc_per_node="$GPUS_PER_NODE" -m molt.cli.train_sft \
  ...
  --logger.wandb.project "${WANDB_PROJECT:-molt_quickstart_sft_qwen3_5_lora}" \
  --logger.wandb.run_name "${WANDB_RUN_NAME:-qwen3_5_sft_lora_quickstart_$$}"
