...
#SBATCH --job-name=molt-sft-qwen3-5-lora
#SBATCH --mem=0
#SBATCH --overcommit
#SBATCH --exclusive

# Qwen3.5-35B-A3B (custom MoE, VLM) LoRA SFT on geo3k.
...
MODEL_PATH="${MODEL_PATH:-/path/to/models/Qwen3.5-35B-A3B}"
...
SAVE_ROOT="${SAVE_ROOT:-$REPO_ROOT/outputs/sft-qwen3-5-lora/$SLURM_JOB_ID}"
...
TRAIN_ARGS=(
  ...
  --logger.wandb.project "${WANDB_PROJECT:-molt_sft_qwen3_5_lora}"
  --logger.wandb.run_name "${WANDB_RUN_NAME:-qwen3_5_sft_lora_$SLURM_JOB_ID}"
)
