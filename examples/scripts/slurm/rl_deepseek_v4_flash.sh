...
#SBATCH --job-name=molt-rl-dsv4-flash
#SBATCH --mem=0
#SBATCH --overcommit
#SBATCH --exclusive
...
# DeepSeek-V4-Flash (model_type=deepseek_v4, DeepseekV4ForCausalLM): ~284B MLA +
# DSA sparse-attention MoE, 43 layers with per-layer compress ratios, 256 routed
# experts (the first layers route by hash). AutoModel custom MoE asserts TP=1.
# 12-node split: 8 actor/ref nodes (64 GPUs, EP64) + 4 vLLM rollout nodes (32
# GPUs); vLLM serves it through its out-of-tree vllm.models.deepseek_v4 backend.
# The checkpoint ships one MTP layer — AutoModel's DSV4 recipes train with
# num_nextn_predict_layers=0, so set that in the checkpoint config to keep the
# MTP head out of the graph.
export MODEL_PATH="${MODEL_PATH:-/path/to/models/DeepSeek-V4-Flash}"
# TP MUST be 1 (custom MoE parallelizer). EP=64 = full non-PP world (dp8*cp8*tp1=64);
# 256 experts / 64 = 4 experts/rank.
export TP_SIZE="${TP_SIZE:-1}"
export EP_SIZE="${EP_SIZE:-64}"
# Activation checkpointing ON (deterministic recompute).
export GRAD_CHECKPOINT="${GRAD_CHECKPOINT-full}"
# CP=8, mirroring AutoModel's DSV4 CP recipe (deepseek_v4_flash_cp_tulu3.yaml):
#  * DSA CP requires attn=tilelang; the runtime capability gate rejects the rest.
#  * That recipe shards FIXED-LENGTH PADDED sequences (packed_sequence_size 0), so
#    packing stays OFF here — each CP rank owns a contiguous shard of MAX_LENGTH.
#    (GLM-5.2 is the opposite: its indexer asserts qkv_format='thd'.)
#  * dp = world 64 / cp8 = 8, so train.batch_size must be >= 8.
#  * cp is the innermost mesh axis, so cp8 = one node: the CP group never crosses
#    the fabric.
...
# Essential to fit the ~284B optimizer state off-GPU without PP.
...
# on these custom-MoE models). Control GPU memory via EP + CP + adam-offload + AC.
export FSDP_CPU_OFFLOAD="${FSDP_CPU_OFFLOAD:-0}"

# DeepSeek-V4 is a TEXT model (DeepseekV4ForCausalLM, no vision) — no visual encoder.
export FREEZE_VISUAL_ENCODER="${FREEZE_VISUAL_ENCODER:-0}"
# Backend mirrors AutoModel's DSV4 recipe: attn=tilelang drives the DSA sparse
# indexer + MLA TileLang kernels. linear=torch, experts=torch_mm (gmm absent);
# molt already defaults rms_norm=torch_fp32, gate_precision=float32,
# rope_fusion=off for non-te — all matching the recipe. dispatcher=torch:
# EP64 spans 8 nodes and internode deepep/hybridep need DOCA/IBGDA, which this
...
# DeepSeek-V4 is MLA+DSA (no GDN recurrent state) — no GDN prefill backend / mamba cache.
# 50-step stability/correctness smoke. max_steps =
# len(dataset)//rollout_batch * num_episodes = 400//8 * 1 = 50.
export LR="${LR:-2e-6}"
# dp=8 (cp8), so train.batch_size must be >= 8 = rollout_batch 8 x n_samples 8.
...
# R3 rollout routing replay ON. Padded (non-packed) CP keeps the routing tensors
# sequence-major, which is what the R3 shard expects.
export ROUTING_REPLAY="${ROUTING_REPLAY:-1}"
# Text math Env agent (DeepSeek-V4 is text-only; the VLM chat_geo3k agent does not
# apply). Point PROMPT_DATASET/EVAL_DATASET at a text dataset to match.
export AGENT_PATH="${AGENT_PATH:-/molt/examples/python/agents/math.py}"
# Bounded run: don't persist a ~284B checkpoint (skip intermediate + final saves).
...
# Actor/ref: 8 dedicated training nodes (64 GPUs, EP64).
export ACTOR_NODES="${ACTOR_NODES:-8}"
# vLLM rollout on the other 4 nodes (32 GPUs). ~284B bf16 (~569GB) does NOT fit one
# node, so 2 engines at TP=16, each spanning 2 nodes; enable_expert_parallel makes
# EP = TP = 16, so the experts shard EP16 (~36 GB/GPU) and MLA attention shards TP16.
# (PP is available via VLLM_PP_SIZE, but PP for RL rollout is unproven.)
...
export SAVE_ROOT="${SAVE_ROOT:-$REPO_ROOT/outputs/async-rl-dsv4-flash/run}"
export WANDB_PROJECT="${WANDB_PROJECT:-molt_rl_deepseek_v4_flash}"
export WANDB_RUN_NAME="${WANDB_RUN_NAME:-dsv4_flash_$SLURM_JOB_ID}"
...
if [ "$CHAIN_DEPTH" -lt "$CHAIN_MAX" ]; then
  ...
  next_jobid=$(CHAIN_DEPTH="$NEXT_DEPTH" LOAD_ENABLE=1 SBATCH_COMMENT="$SBATCH_COMMENT" \
    ...
    "$REPO_ROOT/examples/scripts/slurm/rl_deepseek_v4_flash.sh")
...
# DeepSeek-V4's FSDP wrapper keys each unit's compute dtype off the stored dtype, so
# fp32 master weights would hand the DSA TileLang kernels fp32 and fail. Store bf16,
# matching AutoModel's DSV4 recipes.
if [ "${BF16_MASTER_WEIGHTS:-1}" = "1" ]; then
  RL_ARGS+=(--fsdp.bf16_master_weights)
fi
