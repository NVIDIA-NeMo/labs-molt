...
#SBATCH --job-name=molt-sft-qwen3-30b-a3b-lora
#SBATCH --mem=0
#SBATCH --overcommit
#SBATCH --exclusive

# Qwen3-30B-A3B (stock HF Qwen3MoeForCausalLM) text LoRA SFT.
...
MODEL_PATH="${MODEL_PATH:-/path/to/models/Qwen3-30B-A3B}"
...
SAVE_ROOT="${SAVE_ROOT:-$REPO_ROOT/outputs/sft-qwen3-30b-a3b-lora/$SLURM_JOB_ID}"

GPUS_PER_NODE="${GPUS_PER_NODE:-8}"
MASTER_PORT="${MASTER_PORT:-29500}"

# Qwen3-MoE: TP=1 required by Automodel's custom MoE parallelizer.
...
TRAIN_ARGS=(
  ...
  --logger.wandb.project "${WANDB_PROJECT:-molt_sft_qwen3_30b_a3b_lora}"
  --logger.wandb.run_name "${WANDB_RUN_NAME:-qwen3_30b_a3b_sft_lora_$SLURM_JOB_ID}"
)
